...
async def lookup(ctx):
    ...
    cursor.execute(f"SELECT army FROM data WHERE id = {text}")
    var = cursor.fetchone()
    if var == None:
    ...
    var = var[1:-2]
    army = int(var)
    cursor.execute(f"SELECT recruit FROM data WHERE id = {text}")
    var = cursor.fetchone()
    var = str(var)
    var = var[1:-2]
    recruit = int(var)
    cursor.execute(f"SELECT airship FROM data WHERE id = {text}")
    var = cursor.fetchone()
    var = str(var)
    var = var[1:-2]
    airship = int(var)
    cursor.execute(f"SELECT airPack FROM data WHERE id = {text}")
    var = cursor.fetchone()
    var = str(var)
    var = var[1:-2]
    airPack = int(var)
    cursor.execute(f"SELECT bulletPack FROM data WHERE id = {text}")
    var = cursor.fetchone()
    var = str(var)
    var = var[1:-2]
    bulletPack = int(var)
    cursor.execute(f"SELECT artillery FROM data WHERE id = {text}")
    var = cursor.fetchone()
    var = str(var)
    var = var[1:-2]
    artillery = int(var)
    cursor.execute(f"SELECT artilleryPack FROM data WHERE id = {text}")
    var = cursor.fetchone()
    var = str(var)
    var = var[1:-2]
    artilleryPack = int(var)
    cursor.execute(f"SELECT shield FROM data WHERE id = {text}")
    var = cursor.fetchone()
    var = str(var)
    var = var[1:-2]
    shield = int(var)
    cursor.execute(f"SELECT aa FROM data WHERE id = {text}")
    var = cursor.fetchone()
    var = str(var)
    var = var[1:-2]
    aa = int(var)
    cursor.execute(f"SELECT trench FROM data WHERE id = {text}")
    var = cursor.fetchone()
    var = str(var)
    var = var[1:-2]
    trench = int(var)
    cursor.execute(f"SELECT weaponDamage FROM data WHERE id = {text}")
    var = cursor.fetchone()
    var = str(var)
    var = var[1:-2]
    weaponDamage = int(var)
    cursor.execute(f"SELECT recruitTime FROM data WHERE id = {text}")
    var = cursor.fetchone()
    var = str(var)
    var = var[1:-2]
    recruitTime = int(var)
    cursor.execute(f"SELECT barracks FROM data WHERE id = {text}")
    var = cursor.fetchone()
    var = str(var)
    var = var[1:-2]
    barracks = int(var)
